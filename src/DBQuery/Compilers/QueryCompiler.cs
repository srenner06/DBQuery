...
{
    public delegate string CustomStringValueHandler(StringVal val);

    protected abstract string HandleTableName(string table);
    protected abstract string HandleColumn(string column);
    ...
    protected virtual string HandleCalculatedColumn(string valuie)
    {
        if (valuie.StartsWith('(') && valuie.EndsWith(')'))
            return valuie;
        else
            return $"({valuie})";
    }
    protected virtual string HandleColumnAlias(string original, string alias)
    {
        return $"{original} as {HandleColumn(alias)}";
    }
    protected virtual string HandleCalculatedColumnAlias(string original, string alias)
    {
        return $"{HandleCalculatedValue(original)} as {HandleColumn(alias)}";
    }
    ...
    protected abstract string HandleBlob(BlobVal blobVal);
    protected virtual string HandleQueryVal(QueryVal queryVal, CustomStringValueHandler? stringValueHandler)
    {
        ...
        if (queryVal is BlobVal blobVal)
            return HandleBlob(blobVal);
    ...
    protected virtual void AppendAllColumns<T>(StringBuilder sb, T query) where T : ICalculatedColumn<T>
    {
        ...
        var cols = query.GetAliasColumns() ?? [];
        ...
        else
        {
            if (cols.Length > 0)
            {
                sb.AppendJoin(", ", cols.Select(kp =>
                {
                    if (kp.alias is not null)
                        return HandleColumnAlias(kp.col, kp.alias);
                    else
                        return HandleColumn(kp.col);
                }));
            ...
            if (calculatedfCols.Length > 0)
            {
                sb.AppendJoin(", ", calculatedfCols.Select(kp =>
                {
                    if (kp.alias is not null)
                        return HandleCalculatedColumnAlias(kp.col, kp.alias);
                    else
                        return HandleCalculatedColumn(kp.col);
                }));
            }
        }
    }
    protected virtual void AppendColumns<T>(StringBuilder sb, T query) where T : IColumn<T>
    {
        ...
        {
            if (cols.Length > 0)
                sb.AppendJoin(", ", cols.Select(HandleColumn));
        }
    }
    protected virtual void AppendInsertColumns<T>(StringBuilder sb, T query) where T : IColumn<T>
    {
        ...
        sb.Append(" (");
        sb.AppendJoin(", ", query.GetColumns().Select(HandleColumn));
        sb.Append(')');
    }
        ...
        switch (condition)
        {
            ...
            case ConditionGroup group:
                ...
                for (var i = 0; i < group.Conditions.Count; i++)
                {
                    ...
                    {
                        sb.Append(' ');
                        sb.Append(HandleLogicalOperator(cond.JoinWithPrevious));
                        sb.Append(' ');
                    }

                    AppendCondition(sb, cond, stringValueHandler);
                }
    ...
    protected virtual bool AppendGroupBy<T>(StringBuilder sb, T query) where T : IGroup<T>
    {
        ...
        sb.Append(" GROUP BY ");
        sb.AppendJoin(", ", query.GetGroups().Select(HandleColumn));
        return true;
    }
    ...
    protected virtual bool AppendOrderBy<T>(StringBuilder sb, T query) where T : IOrder<T>
    {
        ...
        sb.Append(" ORDER BY ");
        var orders = query.GetOrders().Select(val => $"{HandleColumn(val.Key)} {(val.Value ? "ASC" : "DESC")}");
    ...
    protected virtual void AppendUpdate<T>(StringBuilder sb, T query, CustomStringValueHandler? stringValueHandler) where T : IUpdate<T>
    {
        ...
        var updates = query.GetUpdates().Select(kp =>
        {
            ...
            return $"{HandleColumn(kp.Key)} = {val}";
        });
    ...
    public string ToQuery<T>(T query, CustomStringValueHandler? stringValueHandler = null) where T : BaseQueryBuilder<T>
    {
        if (query is SelectQueryBuilder select)
            return CompileSelectQuery(select, stringValueHandler);

        if (query is UpdateQueryBuilder update)
            return CompileUpdateQuery(update, stringValueHandler);

        if (query is InsertQueryBuilder insert)
            return CompileInsertQuery(insert, stringValueHandler);

        if (query is DeleteQueryBuilder delete)
            return CompileDeleteQuery(delete, stringValueHandler);
    ...
    #region Select
    public virtual string CompileSelectQuery(SelectQueryBuilder select, CustomStringValueHandler? stringValueHandler = null)
    {
    ...
    #region Update
    public virtual string CompileUpdateQuery(UpdateQueryBuilder update, CustomStringValueHandler? stringValueHandler = null)
    {
    ...
    #region Insert
    public virtual string CompileInsertQuery(InsertQueryBuilder insert, CustomStringValueHandler? stringValueHandler = null)
    {
    ...
    #region Delete
    public virtual string CompileDeleteQuery(DeleteQueryBuilder delete, CustomStringValueHandler? stringValueHandler = null)
    {
